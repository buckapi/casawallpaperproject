...
export class Portfolio implements OnInit {
  ...
  toggleForm() {
    ...
    this.selectedFiles = [];
    this.previews = [];
    this.formData = { name: '', tag: '', instagramUrl: '' };
    this.imagesToDelete = [];
  }
  ...
  editPortfolio(portfolio: any) {
    ...
    this.formData.instagramUrl = portfolio.instagramUrl || '';
    ...
    setTimeout(() => {
      if (this.portfolioForm) {
        this.portfolioForm.setValue({
          ...
          instagramUrl: this.formData.instagramUrl
        });
      }
    });
  }
  ...
  async uploadPortfolio(form: NgForm) {
    ...
    const hasInstagramUrl = !!this.formData.instagramUrl?.trim();

    if (!this.selectedFiles.length && !hasExistingImages && !hasInstagramUrl) {
      Swal.fire({
        icon: 'warning',
        title: 'Media requerida',
        text: 'Sube una imagen/video o agrega una URL de Instagram.',
      });
      return;
    }
    ...
    try {
      ...
      const type: 'img' | 'video' = hasInstagramUrl
        ? 'video'
        : this.selectedFiles.some(file => file.type.startsWith('video/'))
          ? 'video'
          : 'img';

      console.log('Archivos seleccionados:', this.selectedFiles.map(file => ({
        name: file.name,
        type: file.type,
        size: file.size
      })));
      ...
      if (this.isEditing && this.editingPortfolio) {
        ...
        const updateData: any = {
          ...
          tag,
          type,
          sourceType: hasInstagramUrl ? 'instagram' : 'upload',
          instagramUrl: this.formData.instagramUrl?.trim() || ''
        };
        ...
        if (newFiles.length > 0 || this.imagesToDelete.length > 0) {
          ...
          formData.append('sourceType', hasInstagramUrl ? 'instagram' : 'upload');
          formData.append('instagramUrl', this.formData.instagramUrl?.trim() || '');
  ...
  canSubmitPortfolio(): boolean {
    ...
    return hasFiles || hasInstagramUrl;
  }
}